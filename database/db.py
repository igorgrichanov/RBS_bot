...
def add_worker(org, form_list6):
    ...
    try:
        ...
        orgs = org[0][0]
    record = "INSERT INTO `workers` (`id`, `org_id`, `date_add`, `date_upd`, `active`, `name`, `jtitle`, `bet_size`, `jlocation`, `salary`, `date`, `passport_s_num`, `passport_who`, `division_code`, `adress`, `snils`, `inn`, `date_birth`, `employment_history`, `current_account`, `correspondent_account`, `bik_bank`) VALUES (NULL, '" + str(
        orgs) + "', current_timestamp(), current_timestamp(), '1', '" + str(form_list6[0]) + "', '" + form_list6[
                 1] + "', '" + form_list6[2] + "', '" + form_list6[3] + "', '" + form_list6[4] + "', '" + form_list6[
                 5] + "', '" + form_list6[6] + "', '" + form_list6[7] + "', '" + form_list6[8] + "', '" + form_list6[
                 9] + "', '" + form_list6[10] + "', '" + form_list6[11] + "', '" + form_list6[12] + "', '" + form_list6[
                 13] + "', '" + form_list6[14] + "', '" + form_list6[15] + "', '" + form_list6[16] + "');"
    # print(record)
    rec(record)
...
def upd_worker(user_id, val):
    ...
    record = "UPDATE `workers` SET `name` = '" + val[0] + "', `passport_s_num` = '" + val[1] + "', `passport_who` = '" + \
             val[2] + "', `division_code` = '" + val[3] + "', `adress` = '" + val[4] + "', `snils` = '" + val[
                 5] + "', `inn` = '" + val[6] + "', `current_account` = '" + val[7] + "', `correspondent_account` = '" + \
             val[8] + "', `bik_bank` = '" + val[9] + "' WHERE `org_id` = '" + user_id + "';"
    # print(record)
    rec(record)
...
def upd_worker02(user_id, val):
    ...
    record = "UPDATE `workers` SET `name` = '" + val[0] + "', `passport_s_num` = '" + val[1] + "', `passport_who` = '" + \
             val[2] + "', `division_code` = '" + val[3] + "', `adress` = '" + val[4] + "', `snils` = '" + val[
                 5] + "', `inn` = '" + val[6] + "', `current_account` = '" + val[7] + "', `correspondent_account` = '" + \
             val[8] + "', `bik_bank` = '" + val[9] + "' WHERE `id` = '" + user_id + "';"
    # print(record)
    rec(record)
...
def add_log(command, err):
    record = "INSERT INTO `logs` (`id`, `date`, `command`, `errs`) VALUES (NULL, current_timestamp(), '" + command + "', '" + str(
        err) + "');"
    rec(record)
...
def del_org(user_id: str):
    record = f'DELETE FROM `organizations` WHERE `user_id`=\'{user_id}\''
    rec(record)


def update_user_token(user_id: str, token: str):
    is_user_in_db = f'SELECT * FROM `users` WHERE `user_id`=\'{user_id}\''
    resp = rec(is_user_in_db)
    if len(resp) != 0:
        record = f'UPDATE `users` SET `token`=\'{token}\' WHERE `user_id`=\'{user_id}\''
        rec(record)
        return " успешно сгенерирован\n\n"
    else:
        return "Пользователя нет в базе. Убедитесь, что он отправил команду /start"


def check_token(user_id: str):
    record = f'SELECT `token` FROM `users` WHERE `user_id`=\'{user_id}\''
    resp = rec(record)
    if len(resp) != 0:
        token = resp[0][0]
        return token
